...
def monitor_multi_workers(duration: int = 1200):
    ...
    print("-" * 100)
    print(f"{'Timestamp':<20} {'Workers':<10} {'Total CPU %':<15} {'Total Mem (MB)':<15} {'Avg CPU %':<15} {'Avg Mem (MB)':<15}")
    print("-" * 100)
    ...
    try:
        while time.time() - start_time < duration:
            ...
            print(f"{timestamp_str:<20} {worker_count:<10} {total_cpu:<15.2f} {total_mem:<15.2f} {avg_cpu:<15.2f} {avg_mem:<15.2f}")
    ...
    if metrics:
        with open(csv_file, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'timestamp', 'worker_count', 'total_cpu_percent', 'total_memory_mb',
        ...
        if metrics:
            ...
            avg_cpu_per_worker = sum(m['avg_cpu_percent'] for m in metrics) / len(metrics)
